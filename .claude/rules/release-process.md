...
- **Every release must carry a bare `devdeck.exe` asset with exactly that
  name.** `src-tauri/src/update.rs` finds the download URL by matching the asset name
...
- Never create or edit GitHub Releases by hand; never re-tag a published
  version.
- If the GitHub repository moves, update `REPO` in `src-tauri/src/update.rs` in the same
