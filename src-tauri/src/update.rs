...
//! - every release carries a bare `devdeck.exe` asset (exactly that name)
//!
...
pub const REPO: &str = "atman-33/devdeck";
...
/// Returns (tag, download url of the devdeck.exe asset) of the latest release,
...
pub fn check_latest() -> Option<(String, String)> {
    ...
    let url = json
        ...
        .as_array()?
        .iter()
        .find(|a| a.get("name").and_then(|n| n.as_str()) == Some("devdeck.exe"))?
